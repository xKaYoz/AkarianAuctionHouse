...
public class ActiveListingsGUI implements AkarianInventory {
    ...
    public void updateInventory() {
        ...
        if (users.size() > 36 * page) {
            ItemStack next = ItemBuilder.build(Material.NETHER_STAR, 1, AuctionHouse.getInstance().getMessageManager().getMessage(MessageType.BUTTONS_NPAGE_NAME, "%next%;" + (page + 1), "%max%;" + (users.size() % 54 == 0 ? String.valueOf(users.size() / 54) : String.valueOf((users.size() / 54) + 1))),

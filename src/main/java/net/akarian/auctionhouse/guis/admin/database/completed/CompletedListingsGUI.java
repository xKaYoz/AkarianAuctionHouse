...
public class CompletedListingsGUI implements AkarianInventory {
    ...
    public CompletedListingsGUI(Player player, int page) {
        this.player = player;
        this.page = page;
    ...
    public void onGUIClick(Inventory inv, Player p, int slot, ItemStack item, ClickType type) {
        ...
        if (slot >= 8 && slot <= 45 && item != null) {
            ...
            if (container.has(key, new UUIDDataType())) {
                p.openInventory(new PlayerCompletedListings(player, container.get(key, new UUIDDataType()), 1, this).getInventory());
                skullsDisplayed = false;
            }
        }
    }
    ...
    public Inventory getInventory() {
        inv = Bukkit.createInventory(this, 54, chat.format("&2&lCompleted Listings"));
        for (int i = 0; i <= 7; i++) {
    ...
    public HashMap<UUID, ItemStack> loadSkulls() {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(AuctionHouse.getInstance(), () -> {
            ...
            for (UUID uuid : check) {
                ...
                meta.setDisplayName(chat.format("&c&l" + AuctionHouse.getInstance().getNameManager().getName(uuid) + " &e&l(" + AuctionHouse.getInstance().getListingManager().getCompleted(uuid).size() + ")"));
                List<String> lore = new ArrayList<>();
                lore.add("&7Click to view all &e" + AuctionHouse.getInstance().getListingManager().getCompleted(uuid).size() + "&7 completed listings.");
    ...
    public List<UUID> usersWithListings() {
        ...
        for (Listing l : AuctionHouse.getInstance().getListingManager().getCompleted()) {
